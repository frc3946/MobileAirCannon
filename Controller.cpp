...
void Controller::attach(Stream &serial) {
...
bool Controller::update() {
	...
	if(xbee.getResponse().isAvailable()) { //Got Something!
		if (xbee.getResponse().getApiId() == RX_64_IO_RESPONSE) {
			...
			return false; //Expecting Only One Sample
		}
		errorCode = INCORRECT_PACKET_TYPE;
		return false; //Incorrect Packet Type...
	} else if(xbee.getResponse().isError()) {
		errorCode = GENERAL_ERROR;
		return false; //General Error
	}
	errorCode = NO_NEW_PACKET;
	return false; //No New Data...
}

void Controller::printErrorMessage(Stream &serial) {
	switch(errorCode) {
		case GENERAL_ERROR:
			...
			break;
		case NO_NEW_PACKET:
			serial.println("No New Packet Data");
			break;
		case INCORRECT_PACKET_TYPE:
			serial.print("Expected I/O Sample, but got ");
      		serial.print(xbee.getResponse().getApiId(), HEX);
			break;
		case INCORRECT_NUMBER_SAMPLES:
			serial.print("Unexpected Sample size is ");
      		serial.println(ioSample.getSampleSize(), DEC);
			break;
		default:
			serial.println("Unknown, or no, error");
			break;
	}
}
